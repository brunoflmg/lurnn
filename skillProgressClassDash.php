...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <?php
		...
		$query = "SELECT * FROM skills_grade_calculation WHERE student_id='$sid' AND class_id = '$enrolled_class_id1' GROUP BY skills";
	...
	//--Get Class Name---------------------
		...
		$qry = "SELECT * FROM classes WHERE class_id='$enrolled_class_id1'";
		$rest = mysql_query($qry);
		$rwz  = mysql_fetch_object($rest);
		$class_name = $rwz->class_name;
	?>
	<script type="text/javascript">
		...
		function drawChart() 
		{
			...
			var options = {
				title: '<?php echo "Skill Percentage for ".$class_name." class.";?>',
				'width':1150,
					...
					<div class="span3" style="margin:15px 0 0 0; float:right;">   
                        <div class="ui-select" style="width:190px;">
							<select id="progress" onchange="window.location='class_dashboard.php?Qid=<?php echo $enrolled_class_id1;?>&progress='+$('#progress').val();">
								...
								<option value="skill" selected='selected'>Progress By Skill</option>
							</select>
                        </div>
                    </div>
				</div>
				...
				<div class="span12" style="margin:10px 0 0 0">
						...
						<a href="studentupcoming_assignments.php">
							<span style="float:left; width:33%">
								...
								<?php
									...
									//--- Get ESSAY----------------------------------
										$query  = sprintf("SELECT essays.*, class_students.* FROM essays INNER JOIN class_students ON essays.class_id = class_students.class_id Where class_students.email = '$student_email' AND essays.class_id = '$enrolled_class_id1' ORDER BY essays.essay_id DESC");
										$result = mysql_query($query);
					...
					<!----SHOW QUIZ------------>
						<?php
						   ...
						   if(is_array($Gradesfeedquizs) && !empty($Gradesfeedquizs))
							{
								//echo "<pre>"; print_r($feedquizs);
							  ?>
								<?php $i=1;
								   foreach ($Gradesfeedquizs as $assigned_class) {
