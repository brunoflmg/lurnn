<?php
	...
	if(!empty($quiz_id))
	{
      ...
      $query   = sprintf("SELECT t1.* FROM skills as t1 WHERE quiz_id='%d' AND skill_id NOT IN (SELECT skill_id FROM self_assesment WHERE quiz_id='%d' AND student_id='%d')",$quiz_id,$quiz_id,$sid);
      $result  = mysql_query($query);
      ...
      if(!is_array($untaken_skills) || empty($untaken_skills)){
         header("Location: start-quiz-answer.php?id=".$quiz_id);exit;
      }  
    }
      ...
      //d($_POST);
        ...
        $sql = "REPLACE INTO quiz_assigned_students(quiz_id,class_id,creator_tid,student_id,created_date,status) VALUES"."('".$quiz->quiz_id."','".$quiz->class_id."','".$quiz->creator_tid."','".$sid."','".date("Y-m-d H:i:s")."','untaken')";
         ...
         if(mysql_query($query)){
              $success_message ="You have successfully taken the self assessment on the skills. You are being redirected to now to take the quiz within few seconds <script> setTimeout(function(){window.location='start-quiz-answer.php?id=".$quiz->quiz_id."';},1000);</script>";   
         }else{
