<?php
    ...
    if(isset($_POST) && !empty($_POST)){
       //d($_POST,1);
        ...
        if($grade_level!=$user->grade_level && $grade_level!=""){
            $sqlExtra[] = "grade_level='".$grade_level."'";
        }
        ...
        if(isset($sqlExtra) && !empty($sqlExtra)){
            $sql = "UPDATE students SET ".implode(",", $sqlExtra)." WHERE sid='".$user->sid."'";
            //d($sql,1);

            if(mysql_query($sql)){
                $sql    = "SELECT *FROM students WHERE sid='".$user->sid."'";
                $result = mysql_query($sql);
                $user   = mysql_fetch_object($result);
        ...
        if(isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error']==0) {
            ...
            $filename = "student_profile_picture_".$user->user_name.$ext;
            ...
            if($upload_error==false){
                if(move_uploaded_file($_FILES['profile_picture']["tmp_name"],$upload_path . $filename)) {
                    ...
                    $sql = "UPDATE students SET profile_picture='".$filename."' WHERE sid='".$user->sid."'";
                    mysql_query($sql);
        ...
        <div class="container-fluid">
            <div class="settings-wrapper" id="pad-wrapper">
                ...
                <div class="span7 personal-info">
					...
					<a class="btn btn-primary" href="student-offer-classes.php" style="float: right; clear: both;">
                    ...
                    <h5 class="personal-title">Personal info</h5>
                        ...
                        <div class="field-box">
                            ...
                            <div class="ui-select">
                                <select name="grade_level" required="required">
                                    <option value="6th Grade" <?php if($user->grade_level=="6th Grade"){echo 'selected="selected"';}?>>6th Grade</option>
                                    <option value="7th Grade" <?php if($user->grade_level=="7th Grade"){echo 'selected="selected"';}?>>7th Grade</option>
                                    <option value="8th Grade" <?php if($user->grade_level=="8th Grade"){echo 'selected="selected"';}?>>8th Grade</option>
                                    <option value="9th Grade" <?php if($user->grade_level=="9th Grade"){echo 'selected="selected"';}?>>9th Grade</option>
                                    <option value="10th Grade" <?php if($user->grade_level=="10th Grade"){echo 'selected="selected"';}?>>10th Grade</option>
                                    <option value="11th Grade" <?php if($user->grade_level=="11th Grade"){echo 'selected="selected"';}?>>11th Grade</option>
                                    <option value="12th Grade" <?php if($user->grade_level=="12th Grade"){echo 'selected="selected"';}?>>12th Grade</option>
                                </select>
                            </div>
                        </div>
                        <div class="field-box">
                        ...
                        <div class="field-box">
                            ...
                            <input class="span5 inline-input" name="password" type="password" password="password" value="******" />
                        </div>
                        <div class="field-box">
                            <label>Confirm password:</label>
                            <input class="span5 inline-input"  name="confirm" type="password" password="confirm" value="******" />
                        </div>
